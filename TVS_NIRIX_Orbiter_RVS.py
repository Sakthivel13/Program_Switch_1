...
class ResultOverlay(QWidget):
    ...
    def show_result(self, vin: str, is_pass: bool, details: str = ""):
        """Set colors/text and show overlay covering parent window."""
        if is_pass:
            color = "#008000"  # green
            text = f"{vin}\nPASS"
            details = details or "All tests passed successfully."
        else:
            color = "#B00020"  # red
            text = f"{vin}\nFAIL"
        ...
        if not hasattr(self, "message_label"):
            self.message_label = QLabel(self.bottom_panel)
            self.message_label.setAlignment(Qt.AlignCenter)
            self.message_label.setWordWrap(True)
            self.message_label.setStyleSheet("color: black; font-size: 20px;")
            self.message_label.setContentsMargins(28, 10, 28, 10)
            self.bottom_panel.layout().addWidget(self.message_label)

        self.message_label.setText(details)
        self.message_label.setVisible(bool(details))
...
class FlagSelectionPopup(QDialog):
    selected_libraries = pyqtSignal(list)

    def __init__(self, flag_statuses, flag_library_map, parent=None):
        super().__init__(parent)
        self.setWindowTitle("Check Status Flags")
        self.setModal(True)
        self.setFixedSize(900, 420)
        self.flag_statuses = flag_statuses
        self.flag_library_map = flag_library_map
        ...
        header = QLabel("Flag Status Review", self)
        header.setFont(QFont("Segoe UI", 24, QFont.Bold))
        header.setAlignment(Qt.AlignCenter)
        header.setStyleSheet("color: #003B6F;")
        layout.addWidget(header)

        subtitle = QLabel(
            "Review the EOL flag checks below. Select the active libraries you want to test next.", self
        )
        ...
        container = QWidget()
        self.checkbox_layout = QHBoxLayout(container)
        ...
        self.checkboxes = []
        for flag_name, passed in flag_statuses.items():
            card = QFrame()
            card.setStyleSheet(
                f"QFrame {{ background-color: {'#4CAF50' if passed else '#D32F2F'}; "
            ...
            checkbox.setStyleSheet(
                "QCheckBox { color: white; font-size: 18px; font-weight: bold; }"
            )
            ...
            detail.setStyleSheet("color: white; font-size: 14px;")
            card_layout.addWidget(detail)

            self.checkbox_layout.addWidget(card)
            self.checkboxes.append((flag_name, checkbox))

        self.checkbox_layout.addStretch()

        button_row = QHBoxLayout()
        button_row.addStretch()
        apply_button = QPushButton("Apply Selected Libraries", self)
        apply_button.setFixedSize(260, 44)
        apply_button.setCursor(Qt.PointingHandCursor)
        apply_button.setStyleSheet(
            "QPushButton { background-color: #003B6F; color: white; border-radius: 10px; font-size: 16px; }"
    ...
    def apply_selection(self):
        selected = []
        for flag_name, checkbox in self.checkboxes:
            if checkbox.isChecked():
                library_name = self.flag_library_map.get(flag_name, flag_name)
                selected.append(library_name)

        self.selected_libraries.emit(selected)
        self.accept()
...
class ActiveLibrarySelector(QFrame):
    ...
    def add_libraries(self, libraries):
        ...
        for library in libraries:
            if library and library not in existing:
                self.combo.addItem(library)
                existing.add(library)

    def set_libraries(self, libraries, default_value=None):
        self.combo.clear()
        self.combo.addItems(libraries)
        if default_value and default_value in libraries:
            self.combo.setCurrentText(default_value)
        elif libraries:
            self.combo.setCurrentIndex(0)
...
class MainWindow(QWidget):
    ...
    def __init__(self):
        ...
        self.flag_library_mapping = {
            "FTRF": "Flashing_U666",
            "STRF": "Flashing_Static",
            "RTRF": "Orbiter_RVS",
            "DINO": "DINO"
        }
    ...
    def show_final_result_screen(self, is_pass: bool, details: str = ""):
        """Show the overlay until the next VIN scan or manual entry."""
        vin = self.vin_input.text().strip()
        try:
            self.result_overlay.show_result(vin, is_pass, details)
        except Exception as e:
            print(f"Error showing result overlay: {e}")

        # Save results right away
        try:
            QTimer.singleShot(100, self.save_results_to_log)
        except Exception:
            pass

    def _build_failure_overlay_text(self):
        result_text = self.result_box.toPlainText().strip()
        instruction_text = self.instruction_box.toPlainText().strip()
        parts = []
        if result_text:
            parts.append(result_text)
        if instruction_text and instruction_text != result_text:
    ...
    def on_active_library_changed(self):
        ...
        elif active_library == "Check_Status":
            self.vin_input.setPlaceholderText("Scan 17-digit VIN starting with MD6 for status check")
    ...
    def update_test_result_row(self, row_index, actual_value, result):
        active_library = self.active_library_selector.get_selected_library()
        actual_value_col = 6 if active_library in ["3W_Diagnostics", "3W_Battery_Healthcheck", "Flashing_Static", "Flashing_U666", "Orbiter_RVS", "Sedemac_RVS_Station", "BOSCH_CH1_Oil_Filling", "Check_Status"] else 3
        result_col = 7 if active_library in ["3W_Diagnostics","BOSCH_CH1_Oil_Filling", "3W_Battery_Healthcheck", "Flashing_Static", "Flashing_U666", "Sedemac_RVS_Station", "Orbiter_RVS", "Check_Status"] else 4
    ...
    def load_tests_from_sku(self, sku_number, active_library):
        ...
        if active_library == "3W_Battery_Healthcheck":
            ...
            if active_library == "Check_Status":
                full_path = resource_path(os.path.join(self.get_active_library_folder(active_library), test_file_name))
            else:
        ...
        if active_library in ["3W_Diagnostics", "BOSCH_CH1_Oil_Filling", "3W_Battery_Healthcheck", "Flashing_U666", "Flashing_Static", "Sedemac_RVS_Station", "Orbiter_RVS", "Check_Status"]:
        ...
        for idx, row in df.iterrows():
            ...
            columns = ["S.No", "Test Sequence", "Parameter", "Value", "LSL", "USL"] if active_library in ["3W_Diagnostics","3W_Battery_Healthcheck", "Flashing_U666", "Sedemac_RVS_Station", "BOSCH_CH1_Oil_Filling", "Orbiter_RVS", "Check_Status"] else ["S.No", "Test Sequence", "Parameter"]
            ...
            if self.test_table.columnCount() >= 8:
    ...
    def get_active_library_folder(self, active_library):
        if active_library == "Check_Status":
            return "active library(check status)"
    ...
    def get_active_library_path(self, active_library):
        ...
        return os.path.join(base_path, self.get_active_library_folder(active_library))
    ...
    def start_test_cases(self):
        ...
        if self.result_overlay.isVisible():
            ...
            self.result_overlay.hide_result()
            self.reset_for_next_cycle()
            self.vin_input.setText(next_vin)
            self.vin_input.setReadOnly(False)
            self.vin_input.setFocus()
            self.instruction_box.setText('')
    ...
    def _on_worker_error(self, error, duration, logs, row):
        ...
        active_library = self.active_library_selector.get_selected_library()
        self.result = False if active_library == "Check_Status" else error
        self.test_duration = duration
        attempt_time = self.cycle_time_box.seconds
    ...
    def run_next_test(self):
        ...
        if self.current_test_index < len(self.test_cases):
            ...
            if active_library == "Check_Status":
                self.result_box.setText('<span style="color:#003B6F; font-weight:bold; font-size:24px;">Check Status completed. Review flags below.</span>')
                self.instruction_box.setText("Select the flags you want to enable for the next active-library tests.")
                self.show_flag_selection_popup()
                self.result_box.setText('<span style="color:green; font-weight:bold; font-size:24px;">Check Status completed.</span>')
                self.instruction_box.setText("System ready for next VIN number.")
                self.save_results_to_log()
                failure_details = self._build_failure_overlay_text()
                self.show_final_result_screen(self.final_status == "OK", failure_details)
                return
    ...
    def show_flag_selection_popup(self):
        if not self.check_status_results:
            return

        popup = FlagSelectionPopup(self.check_status_results, self.flag_library_mapping, self)
        popup.selected_libraries.connect(self.on_check_status_libraries_selected)
        popup.exec_()

    def on_check_status_libraries_selected(self, selected_libraries):
        if not selected_libraries:
            self.instruction_box.append("No libraries selected from flag review.")
            return

        self.active_library_selector.add_libraries(selected_libraries)
        self.instruction_box.append(
            "Selected libraries are now visible in the Active Library dropdown. "
            "Choose one to continue testing."
        )
    ...
    def _continue_after_worker(self, row):
        ...
        if active_library == "Check_Status":
            self.check_status_results[test_name] = passed
            if not passed:
                self.final_status = "NOK"
            self.instruction_box.append(f"{function_name} {'passed' if passed else 'failed'}")
            if active_library != "Sedemac_RVS_Station":
                QTimer.singleShot(int(time_delay), self._proceed_to_next_test)
            else:
                QTimer.singleShot(1, self._proceed_to_next_test)
