...
class TestWorker(QObject):
    ...
    def __init__(self, library_name, function_name, vin_number, api_url,
                 ...
                 flash_file=None, mcu_dll_file=None, exe_path=None,
    ...
    def run(self):
        ...
        try:
            with redirect_stdout(stream), redirect_stderr(stream): 
                ...
                if self.function_name.lower() == "mcu_flashing":
                    ...
                    try:
                        ...
                        manager = FlashManagerBoschDataOnly(
                            bus_instance=bus,
                            mcu_dll_file_path=self.mcu_dll_file,
                ...
                if self.function_name == "Flag_Update":
                ...
                elif self.function_name == "Hex_file_flashing":
                    result = test_function(
                        self.interface,
                        self.channel,
                        self.bitrate
                        )
                elif self.function_name == "Version_Check":
                    result = test_function(
                        self.interface,
                        self.channel,
                        self.bitrate
                ...
                elif self.function_name == "WIC_file_flashing":
                    result = test_function(
                        self.interface,
                        self.channel,
                        self.bitrate
...
#Select Which Test Sequence from Excel to load
def get_file_name_from_sku(sku_number, active_library):#use library of openpyxl
    mapping_file_path = resource_path("SKU_File_Mapping.xlsx")
    default_sku = "NMB1190010GB"

    try:
        wb = load_workbook(mapping_file_path)#open the excel sheet usethis function(def resource_path(relative_path):)
        ...
        # Required columns(xcel_colums names names defined here only)
        ...
        for row in sheet.iter_rows(min_row=2, values_only=True):
            ...
            sku_list = [s.strip() for s in sku_val.split(",")]#converting all sku number into list format[sku1,sku2,sku3]
...
def get_file_name_from_sku_old(sku_number, active_library):#Pandas library huge,EXE size increase,Startup slow
    mapping_file_path = resource_path("SKU_File_Mapping.xlsx")
    default_sku = "NMB1190010GB"
    import pandas as pd
...
class ResultOverlay(QWidget):# after cycle comple it shows pas or fail (need not to chnage this class)
...
class EmittingStream(io.StringIO):#UI Log window (output is from terminal to nirixx UI)
    ...
    def get_selected_api_url_norton(self, vin=""):
        ...
        try:
            ...
            if "NORTON_META_DATA" not in config or api_key not in config["NORTON_META_DATA"]:
                print(f"Error: Section 'NORTON_META_DATA' or key '{api_key}' not found in {self.api_ini_path}")
            ...
            if vin:
                base_url = base_url.rstrip("/") + f"/{vin}"
            print(f"Selected Norton API URL: {base_url}")
            return base_url
        except Exception as e:
            print(f"[ApiSelector] Error reading '{self.api_ini_path}': {e}")
            self.parent().instruction_box.append(f"Error reading api.ini: {e}. Using default URL.")
...
class ActiveLibrarySelector(QFrame):#excel 3 rd column (Library) on that excel what(Folder) we put thatsaved in station.ini
...
class MainWindow(QWidget):
    ...
    def fetch_flash_file_name_from_api(self, vin_number, api_url):
        try:
            ...
            if "data" not in json_data:
                print(f"[API] ✗ Response missing 'data' key. Available keys: {list(json_data.keys())}")
                return None
            ...
            print(f"[API] Looking for {target_module} flash file...")
            print(f"[API] Total modules in response: {len(modules)}")
            for idx, module in enumerate(modules):
                print(f"[API]   Module {idx}: name='{module.get('module')}', flashfilename='{module.get('flashfilename')}'")
            ...
            for module in modules:
                ...
                if module_name == target_module:
                    ...
                    if flash_file and flash_file.strip():
                        print(f"[API] ✓ {target_module} flash file found: {flash_file}")
                        ...
                        print(f"[API] ⚠ {target_module} module found but flashfilename is empty/missing")
                        return None

            print(f"[API] ✗ {target_module} module not found in response")
            print(f"[API] Available modules: {[m.get('module', 'UNKNOWN') for m in modules]}")
            return None

        except requests.RequestException as e:
            print(f"[API] Request Failed: {e}")
            self.instruction_box.append(f"<span style='color:red;'>API request failed: {e}</span>")
            return None
    ...
    def _start_worker(self, row, function_name):
        ...
        mcu_dll_file = resource_path(
        os.path.join("Sentinel", "MCU.dll")
        )
        ...
        self.worker = TestWorker(active_library, function_name, vin_number, api_url, self.append_to_log_file, interface, channel, bitrate,plc_ip_address,modbus_address, front_mac, rear_mac, flash_file, mcu_dll_file, exe_path, sentinel_code_map=self.sentinel_return_codes)
        self.worker.popup_request.connect(self.show_popup)
        if function_name.lower() == "Hex_File_Flashing":
        ...
        elif function_name.lower() == "Wic_File_Flashing":
            self.flash_dialog = FlashProgressDialog(self)
            self.flash_dialog.show()
            self.worker.flash_progress.connect(self.flash_dialog.update_progress)
    ...
    def _continue_after_worker(self, row):
        ...
        try:
            ...
            if active_library == "3W_Diagnostics":
                ...
                elif test_name in ["Read_Checksum", "Read_VIN","Write_Checksum", "Write_SKU", "Read_SKU", "Write_VIN"]:
                    if isinstance(result, tuple) and len(result) == 3:
                        passed, api_value, actual_value = result
                        self.test_table.setItem(row, 3, QTableWidgetItem(str(api_value)))
                    else:
    ...
    def send_api_status(self):
        vin_number = self.vin_input.text().strip()
        active_library = self.active_library_selector.get_selected_library()
        self.API_URL = "http://172.31.10.21:5000/vehicles/flashFile/updateProcessParams"
