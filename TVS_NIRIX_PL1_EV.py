...
import shutil
import requests
from sentinel import FlashManagerTvs
...
class TestWorker(QObject):
    ...
    def __init__(self, library_name, function_name, vin_number, api_url,
                 ...
                 flash_file=None, vcu_dll_file=None, mcu_dll_file=None, ivcu_dll_file=None, exe_path=None,
        ...
        self.vcu_dll_file = vcu_dll_file
        self.mcu_dll_file = mcu_dll_file
        self.ivcu_dll_file = ivcu_dll_file
    ...
    def run(self):
        ...
        try:
            with redirect_stdout(stream), redirect_stderr(stream): 
                ...
                if self.function_name.lower() == "vcu_flashing":
                    ...
                    def progress_callback(data):
                        ...
                        try:
                            ...
                            # Progress bar
                            try:
                                self.flash_progress.emit(int(progress), message)
                            except Exception:
                                pass
                    ...
                    try:
                        ...
                        manager = FlashManagerTvs(
                            bus_instance=bus,
                            dll_file_path=self.vcu_dll_file,
                            request_canid=int("07B3", 16),
                            response_canid=int("071A", 16),
                            can_trace=True,
                            frame_callback=sys.stdout.write,
                            stdout_progress_callback=progress_callback,
                        )
                        ...
                        self.result_ready.emit(ok, duration, stream.get_logs())
                        ...
                        duration = time.time() - start_time
                        self.result_ready.emit(False, duration, stream.get_logs())
                    finally:
                        if bus is not None:
                            self.log_message.emit("Shutting down CAN bus...")
                            print("Shutting down CAN bus...")
                            bus.shutdown()

                    return
                
                elif self.function_name.lower() == "ivcu_flashing":
                    ...
                    try:
                        ...
                        manager = FlashManagerTvs(
                            bus_instance=bus,
                            dll_file_path=self.ivcu_dll_file,
                            request_canid=int("07B3", 16),
                            response_canid=int("071A", 16),
                            can_trace=True,
                            frame_callback=sys.stdout.write,
                ...
                elif self.function_name.lower() == "mcu_flashing":
                    ...
                    try:
                        ...
                        manager = FlashManagerBoschDataOnly(
                            bus_instance=bus,
                            dll_file_path=self.mcu_dll_file,
                ...
                elif self.function_name in ["Flashing_Flag_Check"]:
                    result = test_function(self.vin_number)
                elif self.function_name in ["Static_Flag_Check"]:
                    result = test_function(self.vin_number)
                elif self.function_name in ["DAQ_Flag_Check"]:
                    result = test_function(self.vin_number)
                    ...
                    if response.status_code == 200:
                        ...
                        for module in modules:
                            for config in module.get("configs", []):
                                # First, look for an explicit message named SKU_WRITE (case-insensitive)
                                for msg in config.get("messages", []):
                                    if (msg.get("refname") or "").upper() == "SKU_WRITE":
                                        sku = msg.get("txbytes")
                                        break
                                if sku:
                                    break
 
                                # Fallback: some APIs put the SKU token in the config refname
                                # (e.g. IVCU_SKU_WRITE, VCU_SKU_WRITE). Accept any refname
                                # that contains 'SKU_WRITE' and pick the first txbytes available.
                                ref = (config.get("refname") or "").upper()
                                if "SKU_WRITE" in ref:
                                    for msg in config.get("messages", []):
                                        if msg.get("txbytes"):
                                            sku = msg.get("txbytes")
                        ...
                        self.show_message_signal.emit(f'<span style="color:red;">Scanned VIN number is not in Selected API Mode ({mode_display}).</span>')
                        self.reset_ui_signal.emit()
                        return
...
class MainWindow(QWidget):
    ...
    def __init__(self):
        ...
        # Flash file caching for current test sequence
        self.cached_flash_file = None
        self.cached_active_library = None
        self.cached_vin_number = None
        
        header_bar = HeaderBar(resource_path("NIRIX Logo.png"), resource_path("TVS logo white.png"))

        log_folder = r"C:\Nirixx\Trace"
        try:
        ...
        self.version = "0526_V1.6.13"

        program_box = InfoBox("iQube/Orbiter", self.version)
        self.cycle_time_box = CycleTimeBox()
        ...
        active_library_default = station_config.get("active_library", "VCU_Flashing_Static")
        ...
        self.load_tests_from_sku("KE190690", self.active_library_selector.get_selected_library())
    ...
    def fetch_flash_file_name_from_api(self, vin_number, api_url):
        try:
            ...
            if not modules:
                ...
                return None
            # Determine target module(s) from active library
            active_library = self.active_library_selector.get_selected_library()
 
            # VCU-based libraries may expose either 'VCU' or 'IVCU' modules
            if "VCU" in active_library:
                target_modules = ["VCU", "IVCU"]
            elif "MCU" in active_library:
                target_modules = ["MCU"]
            else:
                print(f"[API] Library '{active_library}' does not require a flash file.")
                return None
 
            print(f"[API] Looking for flash file in modules: {', '.join(target_modules)}")
            ...
            # Iterate through modules and find any matching target
            for module in modules:
                ...
                if module_name in target_modules:
                    ...
                    if flash_file and flash_file.strip():
                        print(f"[API] ✓ {module_name} flash file found: {flash_file}")
 
                        # Cache module configs for later use (SKU, CVN, CALID, HW_PartNo)
                        ...
                        print(f"[API] ⚠ {module_name} module found but flashfilename is empty/missing")
                        return None
    ...
    def update_test_result_row(self, row_index, actual_value, result):
        active_library = self.active_library_selector.get_selected_library()
        actual_value_col = 6 if active_library in ["3W_Diagnostics", "3W_Battery_Healthcheck", "Flashing_Static", "Flashing_U666", "Orbiter_RVS", "Sedemac_RVS_Station", "BOSCH_CH1_Oil_Filling", "VCU_Flashing_Static", "MCU_Flashing_Static", "EV_RVS"] else 3
        result_col = 7 if active_library in ["3W_Diagnostics","BOSCH_CH1_Oil_Filling", "3W_Battery_Healthcheck", "Flashing_Static", "Flashing_U666", "Sedemac_RVS_Station", "Orbiter_RVS", "VCU_Flashing_Static", "MCU_Flashing_Static", "EV_RVS"] else 4
    ...
    def load_tests_from_sku(self, sku_number, active_library):
        ...
        full_columns_libs = [
            ...
            "Flashing_U666", "Flashing_Static", "Sedemac_RVS_Station", "Orbiter_RVS", "VCU_Flashing_Static", "MCU_Flashing_Static", "EV_RVS"
        ]
    ...
    def on_sku_fetched(self, sku):
        # Clear flash file cache for new test sequence
        self.cached_flash_file = None
        self.cached_active_library = None
        self.cached_vin_number = None
    ...
    def run_next_test(self):
        ...
        if self.current_test_index < len(self.test_cases):
            ...
            #elif test_label.lower() == "vcu_flashing" and active_library == "iQube_Flashing_Static":  
                # Special handling for flashing step
    ...
    def _start_worker(self, row, function_name):
        ...
        if function_name != "Error_Write":
            # CHECK CACHE: Only fetch if library or VIN changed
            if (self.cached_flash_file is None or 
                self.cached_active_library != active_library or 
                self.cached_vin_number != vin_number):
                
                print(f"[CACHE] Fetching flash file for {active_library} (VIN: {vin_number})")
                flash_file_name = self.fetch_flash_file_name_from_api(vin_number, api_url)
                
                if flash_file_name is None and active_library in ["Flashing_U666", "Flashing_Static", "VCU_Flashing_Static", "MCU_Flashing_Static"]:
                    # Log detailed error for MCU debug
                    # error_msg = f"Error while fetching the flash file name for {active_library} - Change the API Mode"
                    # print(f"[ERROR] {error_msg}")
                    # print(f"[DEBUG] Active Library: {active_library}, VIN: {vin_number}")
                    # self.instruction_box.append(error_msg)
                    
                    self.api_helper.show_message_signal.emit(f'<span style="color:red;">Error While Fetching the Flash File Name in this selected API mode.</span>')
                    self.reset_input_and_timer()
                    return
                
                # Update cache
                if flash_file_name:
                    self.cached_flash_file = self.resolve_flash_file(flash_file_name)
                    self.cached_active_library = active_library
                    self.cached_vin_number = vin_number
                    print(f"[CACHE] Cached flash file: {self.cached_flash_file}")
                else:
                    self.cached_flash_file = None
            else:
                print(f"[CACHE] Using cached flash file for {active_library}")
            
            flash_file = self.cached_flash_file
        else:
        ...
        print(f"[DEBUG] Flash file to use: {flash_file}")
        #dll_file = r"D:\TVS NIRIX Flashing\Sentinel\VCU.dll"
        vcu_dll_file = resource_path(os.path.join("Sentinel", "VCU.dll"))
        mcu_dll_file = resource_path(os.path.join("Sentinel", "MCU.dll"))
        ivcu_dll_file = resource_path(os.path.join("Sentinel", "iVCU_U829.dll"))
        ...
        self.worker = TestWorker(active_library, function_name, vin_number, api_url, self.append_to_log_file, interface, channel, bitrate,plc_ip_address,modbus_address, front_mac, rear_mac, flash_file,
                                 vcu_dll_file, mcu_dll_file, ivcu_dll_file, exe_path, sentinel_code_map=self.sentinel_return_codes)
        self.worker.popup_request.connect(self.show_popup)
        if function_name.lower() == "vcu_flashing":
        ...
        elif function_name.lower() == "ivcu_flashing":
            self.flash_dialog = SentinelFlashProgressDialog(self)
            self.flash_dialog.show()
            self.worker.flash_progress.connect(self.flash_dialog.update_progress)
            self.worker.result_ready.connect(self.flash_dialog.close)
        ...
        elif function_name.lower() == "mcu_flashing":
            self.flash_dialog = SentinelFlashProgressDialog(self)
            self.flash_dialog.show()
            self.worker.flash_progress.connect(self.flash_dialog.update_progress)
            self.worker.result_ready.connect(self.flash_dialog.close)
    ...
    def _continue_after_worker(self, row):
        ...
        try:
            ...
            if active_library == "3W_Diagnostics":
            ...
            elif active_library == "VCU_Flashing_Static":
                if isinstance(result, bool):
                    actual_value = "True" if result else "False"
                    passed = result
                ...
                elif test_name in ["Read_Application_SW_Version", "Read_HW_Version","Read_BIN"]:
                    if isinstance(result, tuple) and len(result) == 2:
                        success, version = result
                ...
                elif test_name in ["Read_Checksum", "VIN_Read","Write_Checksum", "Write_SKU", "Read_SKU", "Write_VIN"]:
                    if isinstance(result, tuple) and len(result) == 3:
                        passed, api_value, actual_value = result
                        self.test_table.setItem(row, 3, QTableWidgetItem(str(api_value)))
                    else:
                    ...
                    if isinstance(result, tuple) and len(result) == 2:
                        ...
                        if len(dtc_list) > 0:
                            ...
                            actual_value = (
                                f"{len(dtc_list)} DTC(s): "
                                + ", ".join([dtc.get('code', '') for dtc in dtc_list])
                            )
                            passed = False
                        else:
            ...
            elif active_library == "MCU_Flashing_Static":
                if isinstance(result, bool):
                    actual_value = "True" if result else "False"
                    passed = result
            ...
            elif active_library == "EV_RVS":
                if isinstance(result, bool):
                    actual_value = "True" if result else "False"
                    passed = result
              
            
                elif test_name in ["Read_Software_BOM","Flashing_Flag_Check", "Static_Flag_Check", "DAQ_Flag_Check"]:
                    if isinstance(result, tuple) and len(result) == 2:
                        success, version = result
                        actual_value = version
                        passed = success
                ...
                elif test_name in ["VCU_Read_DTC","Read_DTC"]:
                    if isinstance(result, tuple) and len(result) == 2:
                        passed, dtc_list = result
    ...
    def send_api_status(self):
        ...
        elif active_library in ["VCU_Flashing_Static"]:
            param_pairs = [
                ("CZ14001", "0010"),
                ("CZ14002", "0020")
                ]
            
        elif active_library in ["EV_RVS"]:
            param_pairs = [
                ("CZ14004", "0040")
                ]
            
        elif active_library in ["MCU_Flashing_Static"]:
            param_pairs = [
                ("CZ14101", "0011")
                ]
        ...
        try:
            ...
            for idx in range(self.test_table.rowCount()):
                ...
                try:
                    ...
                    print(f"Error adding Details for test {idx + 1}: {e}")
            ...
            try:
                ...
                print(f"Failed to push API data: {e}")
