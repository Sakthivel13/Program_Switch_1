...
def _extract_ecu_statuses_anywhere(result_dict: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    # Check for single ECU result first
    if "is_active" in result_dict and ("ecu_code" in result_dict or "ecu" in result_dict):
        return [{
            "ecu_code": result_dict.get("ecu_code") or result_dict.get("ecu", ""),
            "is_active": bool(result_dict.get("is_active", False)),
    ...
    if isinstance(rd, dict):
        # Check for single ECU in result_data
        if "is_active" in rd and ("ecu_code" in rd or "ecu" in rd):
            return [{
                "ecu_code": rd.get("ecu_code") or rd.get("ecu", ""),
                "is_active": bool(rd.get("is_active", False)),
                "error_count": 0 if rd.get("is_active") else 1,
        ...
        if isinstance(data, dict):
            # Check for single ECU in data
            if "is_active" in data and ("ecu_code" in data or "ecu" in data):
                return [{
                    "ecu_code": data.get("ecu_code") or data.get("ecu", ""),
                    "is_active": bool(data.get("is_active", False)),
                    "error_count": 0 if data.get("is_active") else 1,
    ...
    for c in candidates:
        ...
        if isinstance(c, dict):
            # details dict format: {"BMS": true/false}
...
def start_auto_run(
    *,
    ...
    try:
        ...
        _log_error(f"Error loading section auto-run config: {e}")
        return {"ok": False, "error": f"CONFIG_LOAD_FAILED: {e}"}

    # Get ECU-level auto-run programs (from ecu_tests.json)
    ecu_programs = []
    try:
        # Get all ECUs for this vehicle
        ecus = list_ecus_for_vehicle(vehicle_name, user_id, user_role)
        for ecu in ecus:
            ecu_code = ecu.get("ecu_code")
            if ecu_code:
                ecu_auto = get_ecu_auto_run_config(vehicle_name, ecu_code)
                if ecu_auto:
                    for prog in ecu_auto:
                        prog["source"] = "ecu"  # Mark as ECU-level
                        prog["ecu_targets"] = [ecu_code]  # Target this ECU
                        ecu_programs.append(prog)
    except Exception as e:
    ...
    for prog_config in sorted(all_programs, key=lambda p: p.get("sort_order", 0)):
        ...
        # Store limits for stream programs
        ...
        specs.append(spec)
        _log_debug(f"Prepared auto-run spec: {program_id} ({spec.program_type}, source={source})")
    ...
    try:
        _update_session_programs_config(session_id, all_programs)
    except Exception as e:
    ...
    def on_single_result_wrapper(sid: str, pid: str, result: Dict[str, Any]):
        ...
        try:
            ...
            if ecu_statuses:
                ...
                # Check if program has ecu_targets
                spec = specs_map.get(pid)
                if spec and spec.ecu_targets:
                    # If program failed, mark targets as inactive
    ...
    # Set stream callbacks on ExecutionContext
...
def get_auto_run_status(session_id: str) -> Dict[str, Any]:
    ...
    if session_id:
        ...
        for prog in programs:
            for ecu in prog.get("ecu_targets", []):
                status = get_ecu_status_from_session(session_id, ecu)
                if status:
    ...
    *,
    ...
    for prog in result.get("programs", []):
        ...
        legacy_programs.append({
            ...
            "source": prog.get("source", "section"),
        })
    ...
    try:
        ...
        for e in discovered:
            ecus.append({
                ...
                "icon": e.get("icon", ""),
                "sort_order": e.get("sort_order", 0),
                "auto_run_programs": e.get("auto_run_programs", []),
            })
    except Exception as e:
