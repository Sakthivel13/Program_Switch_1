    ...
    *,
    ...
    try:
        with contextlib.redirect_stdout(stdout_buf), contextlib.redirect_stderr(stderr_buf):
            ...
            # Verify this is actually a generator
...
def _run_with_retries(
    *,
    ...
    while attempts <= ctx.max_retries:
        ...
        # Ensure stream callback is still set
...
def _extract_ecu_statuses(output: Any) -> List[Dict[str, Any]]:
    ...
    if isinstance(output, dict):
        ...
        # Check for single ECU result (from ecu_active_check.py)
        if "is_active" in output and ("ecu_code" in output or "ecu" in output):
            return [{
...
def _extract_result_value(output: Any) -> Optional[str]:
    ...
    if "is_active" in output_lower:
        return "active" if output_lower["is_active"] else "inactive"
...
class AutoRunSession:
    ...
    def __post_init__(self):
        """Initialize ecu_programs mapping."""
        for prog in self.programs:
            if prog.source == "ecu":
                for ecu in prog.ecu_targets:
                    if ecu not in self.ecu_programs:
                        self.ecu_programs[ecu] = []
                    self.ecu_programs[ecu].append(prog.program_id)
    ...
    def get_ecu_status(self, ecu_code: str) -> Optional[Dict[str, Any]]:
        """Get status for a specific ECU from its programs."""
        with self._lock:
            prog_ids = self.ecu_programs.get(ecu_code, [])
            for pid in prog_ids:
                result = self.results.get(pid)
                if result and result.result_data and isinstance(result.result_data, dict):
                    # Look for ECU status in result data
                    if "is_active" in result.result_data:
                        return {
                            "ecu_code": ecu_code,
                            "is_active": result.result_data.get("is_active", False),
                            "last_response": result.result_data.get("last_response"),
                            "error_count": 0 if result.result_data.get("is_active") else 1,
                        }
                    # Try to extract from output
                    statuses = _extract_ecu_statuses(result.result_data)
                    for s in statuses:
                        if s.get("ecu_code") == ecu_code:
                            return s
            return None
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        with self._lock:
            return {
                ...
                "ecu_programs": dict(self.ecu_programs),
            }
    ...
    def run_auto_session():
        try:
            ...
            for prog in single_programs:
                _log_info(f"  Auto-run single: {prog.program_name} (source={prog.source})")
            ...
            for prog in stream_programs:
                ...
                def make_stream_cb(p_id: str, existing: Optional[Callable] = None):
                    """Create a chained stream callback."""
                    def stream_cb(tid: str, data: Dict[str, Any]):
                        # Call existing callback first (service's DB-persisting)
                        ...
                        # Forward to external on_stream_data callback
...
def get_ecu_status_from_session(session_id: str, ecu_code: str) -> Optional[Dict[str, Any]]:
    ...
    return session.get_ecu_status(ecu_code)
