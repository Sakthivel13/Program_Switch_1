...
Supports split auto-run programs:
...
def get_ecu_details(
    ...
    """Get ECU details from ecu_tests.json, including ECU-specific auto-run programs."""
        ...
        return [
            {
                ...
                "is_active": True,
                "sort_order": 0,
                "auto_run_programs": [],  # Empty if no config
            }
...
def sync_tests_from_filesystem(strict: bool = False) -> Dict[str, int]:
    ...
    for vehicle_dir in list_all_vehicles():
        ...
        try:
            # ── section_tests.json (Section-level auto-run programs) ──
...
# =============================================================================
# DATABASE SYNC — AUTO-RUN PROGRAMS
# =============================================================================
    ...
    if DATABASE_AVAILABLE and query_one is not None:
        try:
            ...
            if db_vehicle:
                ...
                ecu_action = query_one("""
                    SELECT auto_run_programs
                    FROM app.vehicle_diagnostic_actions
                    WHERE vehicle_id = :vid
                      AND ecu_code = :ecu
                      AND is_active = TRUE
                """, {"vid": vehicle_id, "ecu": ecu_code})
            ...
            _log(f"Error fetching ECU auto-run from DB: {e}", "ERROR")

    # Fallback to JSON
    return get_ecu_auto_run_programs(vehicle, ecu_code)
...
def get_vehicle_info(vehicle: str) -> Dict[str, Any]:
    ...
    for ecu in discover_ecus(vehicle):
        ...
        info["ecus"].append({
            "ecu_code": ecu_code,
            "ecu_name": ecu.get("ecu_name", ""),
            "has_auto_run": len(ecu.get("auto_run_programs", [])) > 0,
            "parameters": (
