    ...
    def run(self):
        ...
        try:
            with redirect_stdout(stream), redirect_stderr(stream): 
                ...
                # Map function names to actual module names if needed
                actual_function_name = self.function_name
                actual_module_name = actual_function_name
                
                # Special case: map lowercase variants to actual module names
                function_name_lower = self.function_name.lower()
                if function_name_lower in ["hex_file_flashing", "hex_flashing"]:
                    actual_module_name = "HEX_Flashing"
                    actual_function_name = "HEX_Flashing"
                elif function_name_lower in ["wic_file_flashing", "wic_flashing"]:
                    actual_module_name = "WIC_Flashing"
                    actual_function_name = "WIC_Flashing"
                
                module_name = f"{self.library_name}.{actual_module_name}"
                if module_name in sys.modules:
                    importlib.reload(sys.modules[module_name])
                else:
                    importlib.import_module(module_name)
    
                test_function = getattr(sys.modules[module_name], actual_function_name)
    
                if self.stop_requested:
                    return
                
                # Convert function name to lowercase for case-insensitive comparison
                fn_lower = self.function_name.lower()
                
                if self.function_name == "Flag_Update":
                    result = test_function(self.vin_number)

                elif fn_lower == "tester_present":
                ...
                elif fn_lower in ["hex_file_flashing", "hex_flashing"]:
                ...
                elif fn_lower in ["version_check", "hex_version_check"]:
                    result = test_function(
                        self.interface,
                        self.channel,
                        self.bitrate
                ...
                elif fn_lower in ["wic_file_flashing", "wic_flashing"]:
                    def progress_callback(pct, msg):
                        try:
                            self.flash_progress.emit(int(pct), msg)
                        except Exception:
