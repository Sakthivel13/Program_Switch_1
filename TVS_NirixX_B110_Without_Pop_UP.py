# -*- coding: utf-8 -*-
"""
Created on Wed Jun 10 11:20:07 2026
...
def load_sentinel_return_codes(excel_path=r"D:\TVS NIRIX Flashing\Sentinel\sentinel_return_codes.xlsx"):
    ...
    # Find column indexes
    exit_idx = headers.index("ExitCode")
    msg_idx = headers.index("Message")

    # Read remaining rows
    for row in sheet.iter_rows(min_row=2, values_only=True):
        if row[exit_idx] is not None:
            data[int(row[exit_idx])] = str(row[msg_idx])
...
class TestWorker(QObject):
    ...
    def __init__(self, library_name, function_name, vin_number, api_url,
                 log_callback, interface, channel, bitrate,
    ...
    def run(self):
        print(f"[WORKER] run() start function={self.function_name}, flash_file={self.flash_file}, dll_file={self.dll_file}")
        result = None
        ...
        try:
            with redirect_stdout(stream), redirect_stderr(stream): 
                ...
                if self.function_name.lower() == "mcu_flashing":

                    logs = []

                    def progress_cb(msg, pct, status):
                        if self.stop_requested:
                            return

                        line = f"[{pct:5.1f}%] {msg}"

                        # Emit log (same as before)
                        ...
                        # Emit progress to UI
                        try:
                            self.flash_progress.emit(int(pct), msg)
                        except Exception:
                            pass
                    ...
                    try:
                        import can
                        from sentinel import FlashManagerBoschDataOnly
                        ...
                        manager = FlashManagerBoschDataOnly(
                            bus_instance=bus,
                            dll_file_path=self.dll_file,
                            request_canid=int("07E1", 16),
                            response_canid=int("07E9", 16),
                            progress_callback=progress_cb,
                            flash_timeout_callback=timeout_cb,
                        )

                        ok = manager.flash_ecu(self.flash_file)

                        duration = time.time() - start_time

                        # Equivalent to EXE return code handling
                        if ok:
                            self.log_message.emit("Sentinel Result: Flashing SUCCESS")
                        else:
                            self.log_message.emit("Sentinel Result: Flashing FAILED")

                        # RESULT FLOW SAME AS BEFORE
                        self.result_ready.emit(ok, duration, "\n".join(logs))

                    except Exception as e:
                        err = f"Flashing Exception: {str(e)}"
                        self.log_message.emit(err)
                        print(err)
                        logs.append(err)

                        duration = time.time() - start_time
                        self.result_ready.emit(False, duration, "\n".join(logs))

                    return

                if self.function_name.lower() == "hex_flashing":
                    try:
                        import os
                        b110_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "B110_Cluster_Flashing")
                        if b110_path not in sys.path:
                            sys.path.insert(0, b110_path)

                        from B110_Cluster_Flashing import HEX_Flashing
                        import B110_Cluster_Flashing.config as b110_config
                        import CAN.HW_init as hw_init

                        if self.flash_file:
                            try:
                                if b110_config.HEX_FILES:
                                    b110_config.HEX_FILES[0]["path"] = self.flash_file
                                    print(f"[HEX] Using resolved flash file: {self.flash_file}")
                            except Exception as e:
                                print(f"[HEX] Failed to override HEX file path: {e}")

                        self.log_message.emit("Starting HEX flashing sequence...")
                        self.flash_progress.emit(5, "HEX: CAN init")

                        conn = conn_f = bus = notifier = None
                        try:
                            conn, conn_f, bus, notifier = HEX_Flashing.setup_can("PCAN")
                            self.flash_progress.emit(20, "HEX: ISO-TP ready")
                            ok = HEX_Flashing.run_flash_sequence(
                                conn,
                                conn_f,
                                progress_callback=self.flash_progress.emit,
                            )
                            self.flash_progress.emit(100, "HEX: Complete" if ok else "HEX: Failed")
                            duration = time.time() - start_time
                            self.result_ready.emit(bool(ok), duration, stream.get_logs())
                        finally:
                            try:
                                if notifier:
                                    notifier.stop()
                                ...
                                pass
                            try:
                                hw_init.CanDeInitt(None, bus)
                            except Exception:
                                pass
                    except Exception as e:
                        self.error_occurred.emit(e, time.time() - start_time, stream.get_logs())
                    return
                ...
                elif self.function_name in ["Flag_Update"]:
                    result = test_function(self.vin_number)
                    
                elif self.function_name in ["Hex_Version_Check", "Wic_Version_Check"]:
                    result = test_function(
                        self.interface, self.channel,
                        self.bitrate
                    )
                ...
                elif self.function_name in ["Write_Checksum", "Read_VIN", "Write_SKU", "Read_SKU", "Read_Checksum"]:
                    result = test_function(
                        self.interface, self.channel,
                        self.bitrate, self.api_url, self.vin_number
                    )
                    
                elif self.function_name in ["Write_VIN"]:
                    result = test_function(
                        self.interface, self.channel,
                        self.bitrate, self.vin_number
                    )
                else:
                    result = test_function(
...
def get_file_name_from_sku(sku_number, active_library):
    ...
    try:
        ...
        # Required columns
        sku_col = col_index.get("SKU No")
        file_col = col_index.get("File Name")
        lib_col = col_index.get("Library")

        if sku_col is None or file_col is None or lib_col is None:
        ...
        for row in sheet.iter_rows(min_row=2, values_only=True):
            ...
            file_val = str(row[file_col]).strip() if row[file_col] else ""
            lib_val = str(row[lib_col]).strip() if row[lib_col] else ""

            sku_list = [s.strip() for s in sku_val.split(",")]
        ...
        # 🔍 Match SKU + Library
        for row in data:
            if (row["Library"].lower() == active_library.strip().lower() and
                lookup_sku in row["SKU_List"]):
                return row["File Name"], row["Library"]

        # 🔁 Fallback: SKU match in any library
        for row in data:
            if lookup_sku in row["SKU_List"]:
                return None, row["Library"]

        # 🔁 Default SKU fallback
        for row in data:
            if (default_sku in row["SKU_List"] and
                row["Library"].lower() == active_library.strip().lower()):
                return row["File Name"], row["Library"]
...
def get_file_name_from_sku_old(sku_number, active_library):
    mapping_file_path = resource_path("SKU_File_Mapping.xlsx")
    default_sku = "NMB1190010GB"
    import pandas as pd
                    ...
                    if response.status_code == 200:
                        ...
                        for module in modules:
                            for config in module.get("configs", []):
                                if active_library not in ["Flashing_Static", "Sedemac_RVS_Station", "B110_Cluster_Flashing"]:
                                    name = "VCU_SKU_WRITE"
                                else:
                                    name = "PCM_SKU_WRITE"
...
class ApiSelector(QFrame):
    ...
    def get_selected_api_url_norton(self, vin=""):
        config = configparser.ConfigParser()
        default_url = "http://172.31.10.21:5000/vehicles/flashFile/prd/"
        try:
            if not os.path.exists(self.api_ini_path):
                print(f"[ApiSelector] Error: api.ini not found at {self.api_ini_path}")
                self.parent().instruction_box.append(f"Error: api.ini not found at {self.api_ini_path}. Using default URL.")
            ...
            api_key = self.selected_api.upper()
            if "NORTON_META_DATA" not in config or api_key not in config["NORTON_META_DATA"]:
                print(f"Error: Section 'API' or key '{api_key}' not found in {self.api_ini_path}")
                self.parent().instruction_box.append(f"Error: Section 'NORTON_META_DATA' or key '{api_key}' not found in api.ini. Using default URL.")
                return default_url.rstrip("/") + f"/{vin}" if vin else default_url

            base_url = config["NORTON_META_DATA"][api_key]
            if not base_url:
                print(f"Error: Empty URL for key '{api_key}' in {self.api_ini_path}")
                self.parent().instruction_box.append(f"Error: Empty URL for '{api_key}' in api.ini. Using default URL.")
                return default_url.rstrip("/") + f"/{vin}" if vin else default_url
...
class MainWindow(QWidget):
    ...
    def __init__(self):
        ...
        self.version = "0526_V1.6.11"

        program_box = InfoBox("B110_Cluster_Flashing", self.version)
        self.cycle_time_box = CycleTimeBox()
        ...
        active_library_default = station_config.get("active_library", "B110_Cluster_Flashing")
        ...
        self.load_tests_from_sku("NMB1190010GB", self.active_library_selector.get_selected_library())
    ...
    def stop_current_cycle(self):
        try:
            ...
            if getattr(self, "worker_thread", None):
                self._stop_thread_safe(self.worker_thread, "worker_thread")
    ...
    def reset_for_next_cycle(self):
        try:
            ...
            if worker is not None:
                worker.stop_requested = True  # Signal stop to worker
    
            self._stop_thread_safe(getattr(self, "worker_thread", None), "worker_thread")
            self.worker_thread = None
            self.worker = None
        except Exception as e:
    ...
    def _stop_thread_safe(self, thread, name="thread"):
        ...
        try:
            if thread.isRunning():
                ...
                if not thread.wait(3000):
                    print(f"Warning: {name} did not stop before exit")
        except Exception:
            pass
    ...
    def cleanup_worker_threads(self):
        try:
            ...
            for thread_attr in ("worker_thread", "serial_reader_thread", "thread"):
                thread = getattr(self, thread_attr, None)
                self._stop_thread_safe(thread, thread_attr)
        except Exception as e:
            print(f"Error stopping threads on exit: {e}")
    ...
    def fetch_flash_file_name_from_api(self, vin_number, api_url):
        try:
            print(f"[API] Requesting flash file from: {api_url}")
            ...
            if "data" not in json_data:
                print(f"[API] ✗ Response missing 'data' key. Available keys: {json_data.keys()}")
                return None
                
            modules = json_data.get("data", {}).get("modules", [])
            
            if not modules:
                print(f"[API] ✗ No modules in response")
                return None
    
            # Determine target module from active library
            active_library = self.active_library_selector.get_selected_library()
    
            if "B110" in active_library:
                target_modules = ["IPC"]
            else:
                print(f"[API] Library '{active_library}' does not require a flash file.")
                return None
    
            print(f"[API] Looking for {target_modules[0]} flash file...")
            print(f"[API] Total modules in response: {len(modules)}")
            
            # Print all available modules for debugging
            for idx, mod in enumerate(modules):
                print(f"[API]   Module {idx}: name='{mod.get('module')}', flashfilename='{mod.get('flashfilename')}'")
            
            # Iterate through modules and find target
            for module in modules:
                module_name = module.get("module", "").strip().upper()
                if module_name == target_modules[0]:
                    # Extract flashfilename
                    flash_file = module.get("flashfilename")
                    if flash_file and flash_file.strip():
                        print(f"[API] ✓ {target_modules[0]} flash file found: {flash_file}")
                        
                        configs = module.get("configs", [])
                        if configs:
                            for cfg in configs:
                                ref_name = cfg.get("refname", "")
                                messages = cfg.get("messages", [])
                                if messages:
                                    tx_bytes = messages[0].get("txbytes", "")
                                    print(f"[API]   Config '{ref_name}': {tx_bytes}")
                        
                        return flash_file.strip()
                    else:
                        print(f"[API] ⚠ {target_modules[0]} module found but flashfilename is empty/missing")
                        return None
            
            print(f"[API] ✗ {target_modules[0]} module not found in response")
            print(f"[API] Available modules: {[m.get('module', 'UNKNOWN') for m in modules]}")
            return None
    
        except requests.RequestException as e:
            print(f"[API] Request Failed: {e}")
            self.instruction_box.append(
                f"<span style='color:red;'>API request failed: {e}</span>"
            )
            return None
    ...
    def resolve_flash_file(self, flash_file_name):
        ...
        print(f"[FLASH FILE] resolve_flash_file() local_storage={local_storage}, flash_file_name={flash_file_name}")
    ...
    def update_test_result_row(self, row_index, actual_value, result):
        active_library = self.active_library_selector.get_selected_library()
        actual_value_col = 6 if active_library in ["3W_Diagnostics", "3W_Battery_Healthcheck", "Flashing_Static", "Flashing_U666", "Orbiter_RVS", "Sedemac_RVS_Station", "BOSCH_CH1_Oil_Filling", "B110_Cluster_Flashing"] else 3
        result_col = 7 if active_library in ["3W_Diagnostics","BOSCH_CH1_Oil_Filling", "3W_Battery_Healthcheck", "Flashing_Static", "Flashing_U666", "Sedemac_RVS_Station", "Orbiter_RVS", "B110_Cluster_Flashing"] else 4
    ...
    def load_tests_from_sku(self, sku_number, active_library):
        ...
        try:
            ...
            headers = [
                str(cell.value).strip().replace('\u200b', '') if cell.value else ""
                for cell in sheet[1]
            ]
        ...
        # ✅ Table structure
        full_columns_libs = [
            "3W_Diagnostics", "BOSCH_CH1_Oil_Filling", "3W_Battery_Healthcheck",
            "Flashing_U666", "Flashing_Static", "Sedemac_RVS_Station", "Orbiter_RVS", "B110_Cluster_Flashing"
        ]

        if active_library in full_columns_libs:
        ...
        # ✅ Populate table (FIXED HERE)
        for idx, row in enumerate(data):
            self.test_table.insertRow(idx)

            if active_library in full_columns_libs:
                columns = ["S.No", "Test Sequence", "Parameter", "Value", "LSL", "USL"]
            else:
                columns = ["S.No", "Test Sequence", "Parameter"]

            # Fill main columns
            for col_idx, key in enumerate(columns):
                value = row.get(key, "")
                self.test_table.setItem(idx, col_idx, QTableWidgetItem(str(value)))

            # Add empty columns safely
            if active_library in full_columns_libs:
                self.test_table.setItem(idx, 6, QTableWidgetItem(""))  # Actual Value
                self.test_table.setItem(idx, 7, QTableWidgetItem(""))  # Result
            else:
                self.test_table.setItem(idx, 3, QTableWidgetItem(""))  # Actual Value
                self.test_table.setItem(idx, 4, QTableWidgetItem(""))  # Result
    ...
    def get_battery_name_dynamic(self, battery_number):
        ...
        try:
            ...
            if "Battery Type" not in col_index or "Battery Name" not in col_index:
                raise ValueError("Battery Mapping file is missing required columns.")

            type_col = col_index["Battery Type"]
            name_col = col_index["Battery Name"]

            # Take first 3 characters
            prefix = battery_number[:3].strip()

            # Loop through rows
            for row in sheet.iter_rows(min_row=2, values_only=True):
                battery_type = str(row[type_col]).strip() if row[type_col] else ""
                battery_name = str(row[name_col]).strip() if row[name_col] else ""

                if battery_type == prefix:
                    return battery_name
    ...
    def start_test_cases(self):
        ...
        if (
            ...
            (active_library not in ["3W_Battery_Healthcheck", "B110_Cluster_Flashing"] and (not vin_number.startswith("MD6") or len(vin_number) != 17)or
             active_library == "B110_Cluster_Flashing" and (not vin_number.startswith("SH2") or len(vin_number) != 17))
        ):   
        ...
        api_url = self.api_selector.get_selected_api_url(vin_number)
        Norton_api_url = self.api_selector.get_selected_api_url_norton(vin_number)
        self.url = api_url
        if active_library not in ["B110_Cluster_Flashing"]:
            self.url = api_url
        elif active_library in ["B110_Cluster_Flashing"]:
            self.url = Norton_api_url
        self.cycle_start_time = datetime.now()
        self.cycle_time_box.start_timer()
        if active_library not in ["3W_Battery_Healthcheck", "B110_Cluster_Flashing"]:
            self.fetch_sku_from_api(vin_number, api_url, active_library)
        elif active_library in ["B110_Cluster_Flashing"]:
            self.fetch_sku_from_api(vin_number, Norton_api_url, active_library)
        else:
            self.on_sku_fetched(vin_number[0])  # Just pass first digit

    def run_test(self, library_name, function_name, vin_number, api_url):
        # Use the shared worker naming so central handlers can manage thread lifecycle
        self.worker_thread = QThread()
        self.worker = TestWorker(library_name, function_name, vin_number, api_url)
        self.worker.moveToThread(self.worker_thread)

        self.worker.result_ready.connect(lambda result, duration, logs: self._on_worker_result(result, duration, logs, self.current_test_index, function_name))
        self.worker.error_occurred.connect(lambda error, duration, logs: self._on_worker_error(error, duration, logs, self.current_test_index))
    ...
    def run_next_test(self):
        ...
        if self.current_test_index < len(self.test_cases):
            ...
            #elif test_label.lower() == "mcu_flashing" and active_library == "B110_Cluster_Flashing":  
                # Special handling for flashing step
    ...
    def _start_worker(self, row, function_name):
        ...
        base_url = config.get("NORTON_META_DATA", selected_mode, fallback=None)
        api_url = self.url
        flash_file=None
        flash_file_name = self.fetch_flash_file_name_from_api(vin_number, api_url)
        if function_name != "Error_Write":
            ...
            if flash_file_name is None and active_library in ["Flashing_U666", "Flashing_Static", "B110_Cluster_Flashing"]:
                self.instruction_box.append("Error while fetching the flash file name.")
                self.reset_input_and_timer()
                return
        else:
        ...
        print(flash_file)
        #dll_file = r"D:\TVS NIRIX Flashing\Sentinel\MCU.dll"
        dll_file = resource_path(os.path.join("DLL_File", "VIC_Cluster.dll"))
        #exe_path = r"D:\TVS NIRIX Flashing\Sentinel\SentinelCLI.exe"
        exe_path = resource_path(os.path.join("Sentinel", "SentinelCLI.exe"))
        config_data = load_station_config()
        interface = config_data.get("interface", "N/A")
        channel = config_data.get("channel", "N/A")
        bitrate = config_data.get("bitrate", "N/A")
        ...
        if isinstance(old_thread, QThread) and old_thread is not None:
            self._stop_thread_safe(old_thread, "previous worker_thread")
            self.worker_thread = None
            self.worker = None

        self.worker_thread = QThread()
        print(f"[MAIN] Starting worker thread for function={function_name}, flash_file={flash_file}")
        self.worker = TestWorker(active_library, function_name, vin_number, api_url, self.append_to_log_file, interface, channel, bitrate, front_mac, rear_mac, flash_file, dll_file, exe_path, sentinel_code_map=self.sentinel_return_codes)
        self.worker.popup_request.connect(self.show_popup)
        if function_name.lower() == "hex_flashing":
            # Run HEX_Flashing without showing the popup UI.
            # Keep the progress signal available for logging or other handlers.
            pass
        elif function_name.lower() == "wic_flashing":
            self.flash_dialog = FlashProgressDialog(self)
            self.flash_dialog.show()
            self.worker.flash_progress.connect(self.flash_dialog.update_progress)
    ...
    def _continue_after_worker(self, row):
        ...
        try:
            ...
            elif active_library == "B110_Cluster_Flashing":
                if isinstance(result, bool):
                    actual_value = "True" if result else "False"
                    passed = result
            
                elif test_name in ["Hex_Version_Check","Wic_Version_check"]:
                    if isinstance(result, tuple) and len(result) == 2:
                        success, version = result
                        actual_value = str(version) if version is not None else ""
                        passed = success
                    else:
                        actual_value = "Error"
                        passed = False
        ...
        if passed:
            ...
            self.retry_count += 1
            if self.retry_count < self.max_retries:
                self.instruction_box.append(f"{function_name} failed on attempt {self.retry_count}. Retrying...")
                QTimer.singleShot(2000, lambda: self._start_worker(row, function_name))
            else:
                self.test_failed = True
                self.final_status = "NOK"
                self.instruction_box.append(f"{function_name} failed after {self.max_retries} retries.")
                self.progress_bar.setValue(100)
                self.send_api_status()
                self.test_cycle_completed = True
                self.cycle_time_box.stop_timer()
...
if __name__ == "__main__":
    ...
    window = MainWindow()
    app.aboutToQuit.connect(window.cleanup_worker_threads)
    window.show()
    sys.exit(app.exec_())
